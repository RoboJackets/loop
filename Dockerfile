...
RUN set -eux && \
    ...
    apt-get install -qq --assume-yes \
        ...
        unzip libfcgi-bin default-mysql-client zopfli php8.5-redis php8.5-ldap poppler-utils file php8.5-zip git && \
    ...
    sed -i '/upload_max_filesize/c\upload_max_filesize = 40M' /etc/php/8.5/fpm/php.ini && \
    sed -i '/post_max_size/c\post_max_size = 40M' /etc/php/8.5/fpm/php.ini && \
    sed -i '/max_file_uploads/c\max_file_uploads = 1' /etc/php/8.5/fpm/php.ini && \
